...
buttonAgregarPagina.onclick = function(){
    ...
    if(!contenedorVerificando){
        ...
        contenedor.setAttribute("style", "border:solid 1 px black; height:500px; overflow: scroll; overflow-x: hidden;");

        pagina.appendChild(contenedor);

    }else{
        alert("Ya se agrego el contenedor a la pagina");
    }
};
...
buttonTitulo.onclick = function(){
    ...
    if(contenedor){
        ...
        alert("Primero debe agregar un contenedor a la pagina");
    }
    ...
    if(contenedor){
        if (menu.length > 0){
            ...
            if(texto != "" && texto !=null){
                ...
                alert("No se ha registraso ningun parrafo, por favor ingrese informacion");
            }
        }else{
            alert("Debe agregar un menu primero");
        }
